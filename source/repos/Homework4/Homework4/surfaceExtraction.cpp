...
void writeTriangles(FaceType face, FILE* filePointer, int x, int y, int z)
{
	Triangle tri1[1];
	Triangle tri2[1];
	...
	checkTriangles(tri2);*/
	extractFace(x, y, z, face, tri1[0], tri2[0]);
...
void writeSTL(VoxelModel model, const char* filename)
{
	...
	int x = 0, y = 0, z = 0, length;
	length = model.xDimension * model.yDimension * model.zDimension * 8;
	for (int i = 0; i < length; i++)
	{
		...
		{
			//X axis
			if (!getBit(model, x - 1, y, z))
			{
				FaceType face = NX;
				writeTriangles(face, filePointer, x, y, z);
				numTriangles[0] += 2;
			}
			if (!getBit(model, x + 1, y, z))
			{
