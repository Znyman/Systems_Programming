...
{
	assert(numThreads > 0 && arraySize > 0);
	double sum = 0.0;
	...
	for (int i = 0; i < numThreads; i++)
	{
		if (i < numThreads - 1)
		{
			threads[i] = new std::thread(SumValues, array, &sums[i], i * range, (i + 1) * range);
		}
		else
		{
			threads[i] = new std::thread(SumValues, array, &sums[i], i * range, arraySize);
		}
	}
